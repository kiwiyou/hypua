        ...
        loop {
            ...
            match non_pua_index {
                Some(non_pua_index) => {
                    ...
                    pua.chars().flat_map(to_ipf).for_each(|ipf| buffer.push_str(ipf));
                    pua_str = non_pua;
                }
                None => {
                    pua_str.chars().flat_map(to_ipf).for_each(|ipf| buffer.push_str(ipf));
...
pub trait IntoIpfIterator<'a> {
    ...
    /// 
    /// let text = "믌";
    /// let char_vec: Vec<char> = text.into_ipf_iter().collect();
